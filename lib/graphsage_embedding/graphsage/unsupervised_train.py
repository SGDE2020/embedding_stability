...
flags.DEFINE_integer('seed', None, "Random seed for tensorflow and numpy")
...
def train(train_data, test_data=None):
    ...
    if FLAGS.save_embeddings:
        ...
        if FLAGS.model == "n2v":
            ...
            train_ids = tf.constant([[id_map[n]] for n in G.nodes_iter() if not G.node[n]['val'] and not G.nodes[n]['test']],
                    dtype=tf.int32)
            test_ids = tf.constant([[id_map[n]] for n in G.nodes_iter() if G.node[n]['val'] or G.nodes[n]['test']], 
                    dtype=tf.int32)
